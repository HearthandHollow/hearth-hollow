...
> **Status (set up via Retell API):** Agent and function are already created and
> verified end-to-end.
> - Retell agent: `agent_d8261e19625fba1ebfcd44896f` ("Hearth & Hollow - Quote Intake")
> - Retell LLM: `llm_b1b4dc798507a7a5a481914a0669`
> - Function `submit_quote_request` → `https://www.thehearthhollow.com/api/voice/quote`
> - Endpoint deployed, `RETELL_WEBHOOK_SECRET` set in Vercel, test submission returned 201.
> - **Remaining:** assign a phone number to the agent (Retell → Phone Numbers),
>   then delete any test quotes from `/admin/quotes`.
>
> **Important — use the `www` URL.** `thehearthhollow.com` 308-redirects to
> `www.thehearthhollow.com`, and most HTTP clients drop the `Authorization`
> header across that redirect (→ 401). Always point Retell at the `www` URL.
...
Caller ──▶ Retell voice agent ──▶ custom function "submit_quote_request"
                                        │  (JSON + bearer secret)
                                        ▼
              POST https://www.thehearthhollow.com/api/voice/quote
                                        │
                                        ▼
...
```bash
curl -i -X POST https://www.thehearthhollow.com/api/voice/quote \
...
- **URL:** `https://www.thehearthhollow.com/api/voice/quote`
- **Method:** `POST`
- **Headers:**
